...
FogUI is a full-stack generative UI platform. The core of the project is in `backend-java` and `packages/react`.
...
- **`client`** and **`dashboard`**: These are example/mock React applications that consume the backend and the `@fogui/react` package. Do not focus development here unless specified. They are good for seeing usage examples.
